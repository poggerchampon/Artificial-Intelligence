...
class Localizer:
    ...
    # add your simulator and filter here, for example    
        ...
        self.__HMM = RobotSimAndFilter.HMMFilter(self.__sm, self.__om, self.__tm, self.__fVec)
        self.error = 0
    #
    ...
    def update(self) -> (bool, int, int, int, int, int, int, int, int, np.array(1)) :
        ...
        self.__trueState = self.__rs.move()
        self.__sense = self.__rs.sense_in_current_state()
        guess, self.__fVec = self.__HMM.filter(self.__sense)
        ...
        eX, eY = self.__sm.state_to_position(guess)
        
        # this should be updated to spit out the actual error for this step
        self.error = np.absolute(tsX-eX) + np.absolute(tsY-eY)            
        ...
        return ret, tsX, tsY, tsH, srX, srY, eX, eY, self.error, self.__fVec
    ...
    def eval(self):
        ...
        rand_err = 0
        rand = 0
        num_states = self.__sm.get_num_of_states()
        for i in range(200):
            rand = random.randrange(num_states)
            ret, tsX, tsY, tsH, srX, srY, eX, eY, error, fVec = self.update()
            ranX , ranY = self.__sm.state_to_position(rand)
            err += error
            if tsX == eX and tsY == eY:
                correct_guess += 1
            if ret:
                sense_count += 1
                sense_err += np.absolute(tsX-srX) + np.absolute(tsY-srY)
                if tsX == srX and tsY == srY:
                    corr_sense +=1
            if tsX == ranX and tsY == ranY:
                corr_rand += 1
            rand_err += np.absolute(tsX-ranX) + np.absolute(tsY-ranY)
        err = err/200
        rand_err = rand_err/200
        sense_err = sense_err/sense_count
        return err, correct_guess, sense_err, sense_count, corr_sense, rand_err,corr_rand